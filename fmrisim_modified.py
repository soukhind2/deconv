...
NOTE: THIS MODULE IS ORIGINALLY PART OF BRAINIAK AND HAS BEEN MODIFIED FOR USE.
WE RECOGNIZE THE METHODS DEVELOPED BY ELLIS ET. AL. 2020 AND EXTEND THEM IN OUR WORK.
PLEASE SEE: 
...
def linear(f,g):
    '''
        ...
        Linear time course
        -------
        Added later. NOT WITH FMRISIM
        Helper for stim_convolve
    '''
...
def convolve_hrf(stimfunction,
    ...
    hrf_type : str or list
        ...
        Other options are 'custom' and 'square'
    params: dict
    ...
    nonlin: Default: False
    
        If set to true, it performs nonlinear convolution
    Returns
    ----------
    ...
    if hrf_type == 'double_gamma' or None:
    ...
    for list_counter in range(list_num):
        ...
        if nonlin:
            signal_temp = stim_convolve(stimfunction[:, list_counter], hrf)  #Major change,
        else:
            signal_temp = np.convolve(stimfunction[:, list_counter], hrf)  #Major change,    
        #this is the part where non linearity is introduced
...
def _calc_ARMA_noise(volume,
    ...
    for voxel_counter in range(sample_num):
        ...
        try:
            model = ARIMA(demeaned_timecourse, [auto_reg_order, ma_order])
