...
plt.plot(avgc2[0],'-D',label = '0',c = 'red')
plt.plot(avgc2[1],'-o',label = '0.1',c = 'black')
plt.plot(avgc2[2],'-o',label = '0.2',c = 'orange')
plt.plot(avgc2[3],'-x',label = '0.3',c = 'lime')
plt.plot(avgc2[4],'-h',label = '0.4',c = 'violet')
plt.plot(avgc2[5],'-*',label = '0.5',c = 'blue')
plt.legend(loc = 4,title = 'Proportion')
#ax.set_xlabel('Upper Bound of SOA')
ax.set_ylabel('Estimation Efficiency',size = 15)
#ax.set_ylabel('HRF Estimation Eff.')
#ax.set_title('HRF Estimation Eff. for Different Cue Only Trial Proportions(lsoa = 4)')
ax.set_title('Estimation Efficiency for Semi - Null Trial Proportions(lSOA = 2)',size = 15)
labels = uisi.astype(str)
labels[0] = '2s Const. SOA'
